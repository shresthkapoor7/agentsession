...
function viewerDocument(transcript: Transcript, { sessionCount = 1, sessionTabs = [], sourceSessions = [], summaryOnly = false }: { sessionCount?: number; sessionTabs?: SessionTab[]; sourceSessions?: Transcript[]; summaryOnly?: boolean } = {}) {
  ...
  const usageByTurn = completedTurns.map((turn, index) => {
    ...
    if (!summaryOnly && Number.isFinite(startMs)) {
      for (let i = 0; i < groupStartMs.length; i++) {
        if (groupStartMs[i] <= startMs) groupIndex = i;
        else break;
      }
    }
    return { groupIndex, index: index + 1, processed, turn };
  });
  ...
  const bars = usageByTurn.length
    ? usageByTurn.map(({ groupIndex, index, processed, turn }) => {
      const content = `<span>${index}</span><div class="usage-bar-track"><div class="usage-bar" style="height:${Math.max(8, Math.round((processed / peakTurn) * 100))}%" title="Task ${index}: ${tokenLabel(processed)} processed tokens in ${durationLabel(turn.durationMs ?? 0)}"></div></div><b>${tokenLabel(processed)}</b>`;
      return groupIndex === null
        ? `<div class="usage-bar-item">${content}</div>`
        : `<button aria-label="Open conversation for task ${index}" class="usage-bar-item usage-bar-jump" data-usage-group-index="${groupIndex}" type="button">${content}</button>`;
    }).join("")
